...
static const char *const ETA_FORMAT = "%2d%% - %2dh%02dm%02ds";
/// The maximum number of characters that the ETA_FORMAT can ever yield
enum { ETA_FORMAT_LENGTH  = 25 };
...
static void progressbar_draw(const progressbar *bar)
{
  ...
  // Draw the ETA
  fputc(' ', stderr);
  fprintf(stderr, ETA_FORMAT, bar->percent, eta.hours, eta.minutes, eta.seconds);
  fputc('\r', stderr);
}
